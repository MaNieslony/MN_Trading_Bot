...
def price_4leg_credit(
    *,
    ...
    """
    Price a 4-leg credit combo (Iron Condor). If `tickers` is provided, it will
    be used and no reqTickers call is made.
    Returns (credit_mid, credit_natural) in IB-style NEGATIVE credits.
    """
    if tickers is None:
        if req_tickers_callable is None or get_option_conid_callable is None:
            raise ValueError(
                "price_4leg_credit requires get_option_conid_callable and req_tickers_callable when tickers is None"
            )
        if wait_for_ticker_data_callable is None:
            raise ValueError("price_4leg_credit requires wait_for_ticker_data_callable when tickers is None")
        ...
        if not c1 or not c2 or not c3 or not c4:
            if logger:
                logger.warning("4-leg: failed to qualify one or more option contracts")
            return None, None

        tickers = req_tickers_callable(c1, c2, c3, c4)

        if not wait_for_ticker_data_callable(tickers, timeout=timeout):
            if logger:
                logger.warning("4-leg: ticker timeout")
            return None, None
    ...
    legs = [
        ...
        {"action": leg2_action, "qty": leg2_qty, "ticker_index": 1},
        {"action": leg3_action, "qty": leg3_qty, "ticker_index": 2},
        {"action": leg4_action, "qty": leg4_qty, "ticker_index": 3},
    ]
    ...
    if credit_mid is None or credit_natural is None:
        if logger:
            logger.warning("4-leg: invalid bid/ask data")
        return None, None
