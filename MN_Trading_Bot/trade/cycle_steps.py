...
from market.rut_ic_steering import select_rut_iron_condor, resolve_dte_and_delta_from_iv_rank, is_late_entry
...
def select_legs(bot, expiry: str, strikes: List[float], deltas: Dict[float, float], rescan: int, trading_class: str):
    ...
    if bot.TRADE_TYPE == "IRON_CONDOR":
        ...
        metrics.short_delta = result.get("short_put_delta")  # backward-compat für CSV/Telegram
